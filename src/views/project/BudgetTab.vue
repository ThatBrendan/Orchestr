  ...
  <div class="fade-in">
    ...
    <template v-else-if="financials">
      <div class="border rounded-xl px-6 py-5 border-line bg-surface grid sm:grid-cols-3 gap-x-8 gap-y-5">
        ...
        />
        <StatTile
          :value="format(financials.committed_spend_minor, currency)"
        ...
        />
        <StatTile
          :value="format(financials.outstanding_minor, currency)"
          label="Outstanding"
        />
        <StatTile
      ...
      <div
        v-if="hasTarget"
        class="mt-5 border rounded-xl px-6 py-4 border-line bg-surface grid sm:grid-cols-2 gap-4"
      >
        <div>
          ...
          <div
            class="text-14 font-medium mt-0.5"
            :class="(financials.projected_variance_minor ?? 0) < 0 ? 'text-amber' : 'text-ink'"
          >
            {{ varianceLabel(financials.projected_variance_minor) }}
          </div>
          <div class="text-13 text-muted mt-0.5">
            Target vs. committed spend (open activities included)
          </div>
        </div>
        <div>
          <div class="text-13 text-muted">
          ...
          <div
            class="text-14 font-medium mt-0.5"
            :class="(financials.settled_variance_minor ?? 0) < 0 ? 'text-amber' : 'text-ink'"
          >
            {{ varianceLabel(financials.settled_variance_minor) }}
          </div>
          <div class="text-13 text-muted mt-0.5">
            Target vs. money actually paid so far
          </div>
        </div>
      </div>
      <p
        v-else
        class="mt-4 text-13 text-muted"
      >
        No total budget has been set. Budget target editing is not available in this build.
      </p>
      ...
      <div class="mt-8">
        ...
        <EmptyState
          ...
          class="mt-3"
          message="No spend or category targets recorded yet."
        />
        <div
          v-else
