  ...
  <PageContainer>
    <div class="flex items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-1.5 text-14.5 text-ink-soft">Everything you're currently planning.</p>
      </div>
      <AppButton @click="showNew = true"><AppIcon name="plus" :size="16" />New project</AppButton>
    </div>

    <div class="mt-8 space-y-3">
      <template v-if="isPending">
        <SkeletonBlock v-for="i in 3" :key="i" height="120px" rounded="0.75rem" />
      </template>
      ...
      >
        <template #action><AppButton @click="showNew = true">New project</AppButton></template>
      </EmptyState>
      <ProjectSummaryRow v-for="p in projects" v-else :key="p.project_id" :project="p" />
    </div>

    <NewProjectDialog :open="showNew" @close="showNew = false" />
  </PageContainer>
</template>
