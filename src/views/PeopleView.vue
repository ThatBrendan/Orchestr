...
<template>
  <PageContainer width="lg">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        <h1 class="font-display text-[26px] font-semibold tracking-tight">People</h1>
        <p class="mt-1.5 text-14.5 text-ink-soft">Everyone you plan with across your projects.</p>
      </div>
    ...
    <div v-if="isPending" class="mt-6 space-y-2">
      <SkeletonBlock v-for="i in 5" :key="i" height="72px" rounded="0.75rem" />
    </div>
    ...
    />
    <EmptyState v-else-if="filteredPeople.length === 0" class="mt-6" message="No people match those filters." />

    <div v-else class="mt-6 divide-y rounded-xl border border-line bg-surface">
      <button
      ...
      >
        <AppAvatar :name="person.display_name" :url="person.avatar_url" :size="40" />
        <div class="min-w-0 flex-1">
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-14 font-medium">{{ person.display_name }}</span>
            <StatusBadge v-if="person.is_current_user" label="You" tone="accent" />
            <StatusBadge v-if="!person.user_id" label="Name-only member" tone="neutral" />
          </div>
          <div class="mt-0.5 text-13 text-muted">{{ projectLabel(person.project_count) }}</div>
          <div class="mt-1 flex flex-wrap gap-1.5">
            ...
            <span v-if="person.projects.length > 3" class="px-2 py-0.5 text-12 text-muted">+{{ person.projects.length - 3 }}</span>
          </div>
        </div>
      </button>
    </div>

    <div v-if="selectedPerson" class="fixed inset-0 z-40 bg-black/20 px-4 py-6" @click.self="selectedPerson = null">
      <aside class="ml-auto flex h-full w-full max-w-md flex-col rounded-xl border border-line bg-surface shadow-xl">
        <div class="flex items-start gap-3 border-b border-line p-5">
          <AppAvatar :name="selectedPerson.display_name" :url="selectedPerson.avatar_url" :size="44" />
          <div class="min-w-0 flex-1">
            <h2 class="font-display text-[21px] font-semibold tracking-tight">{{ selectedPerson.display_name }}</h2>
            <div class="mt-1 flex flex-wrap gap-2">
              <StatusBadge v-if="selectedPerson.is_current_user" label="You" tone="accent" />
              <StatusBadge v-if="!selectedPerson.user_id" label="Name-only member" tone="neutral" />
            </div>
          </div>
          <AppButton variant="secondary" size="sm" @click="selectedPerson = null">Close</AppButton>
        </div>
        <div class="flex-1 overflow-y-auto p-5">
          <h3 class="text-13 font-semibold uppercase tracking-wide text-ink-soft">Shared projects</h3>
          <div class="mt-3 space-y-2">
            ...
            >
              <div class="flex items-center justify-between gap-3">
                ...
                <StatusBadge :label="roleLabel(project.role)" :tone="roleTone(project.role)" />
              </div>
              <div v-if="project.status !== 'active'" class="mt-1 text-13 capitalize text-muted">{{ project.status }}</div>
            </RouterLink>
          </div>
        </div>
      </aside>
