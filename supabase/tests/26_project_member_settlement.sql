...
select pg_temp.login('aa250000-0000-0000-0000-000000000025','settlement-a@t.co');
create temporary table pg_temp.second as with c as (insert into public.commitments(project_id,title,kind,activity_type,status,estimated_cost_minor,cost_split_mode)
select project_id,'Second','technology','task','idea',1000,'none' from pg_temp.ctx returning id) select * from c;
select public.set_activity_cost_split((select id from pg_temp.second),jsonb_build_object('mode','even','cost',1000,'members',jsonb_build_array((select id from pg_temp.recipient))));
select public.record_member_payment((select id from pg_temp.second),(select id from pg_temp.recipient),500,'installment',true,current_date);
select is((select allocated_minor from public.v_project_member_settlements where project_id=(select project_id from pg_temp.ctx)),4000::bigint,'Multiple Activity allocations aggregate');
select is((select remaining_minor from public.v_project_member_settlements where project_id=(select project_id from pg_temp.ctx)),500::bigint,'Partial project balance');
select public.record_member_payment((select id from pg_temp.item),(select id from pg_temp.recipient),200,'refund',true,current_date);
select is((select remaining_minor from public.v_project_settlement_totals where project_id=(select project_id from pg_temp.ctx)),700::bigint,'Refund updates project remaining');
update public.project_members set status='removed' where id=(select id from pg_temp.recipient);
select is((select member_status::text from public.v_project_member_settlements where project_id=(select project_id from pg_temp.ctx)),'removed','Removed member history retained');
select pg_temp.login('cc250000-0000-0000-0000-000000000025','settlement-c@t.co');
select is((select remaining_minor from public.v_project_member_settlements where project_id=(select project_id from pg_temp.ctx)),700::bigint,'Viewer reads member balances');
select pg_temp.login('dd250000-0000-0000-0000-000000000025','settlement-d@t.co');
select is((select count(*)::int from public.v_project_member_settlements where project_id=(select project_id from pg_temp.ctx)),0,'Project member balance isolation');
select * from finish();
rollback;
